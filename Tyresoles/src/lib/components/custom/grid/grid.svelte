<script lang="ts">
  ...
  let currentSort: { column: string; direction: "desc" | "asc" } | undefined =
    $state();
  ...
  const prepareSelection = () => {
    ...
    selectedValues.forEach((value) => {
      const matchingRow = data.find(
        (r) => getObjectValues(r, dataKey ?? "code") === value
      );
      if (matchingRow) {
...
<div
  id="grid-main"
  class={cn(
    "flex flex-1 flex-col gap-1 rounded-sm ",
    type !== "dropdown" && `p-1`
  )}
...
>
  {#if type !== "dropdown"}
    <div class="md:flex-rows flex flex-col gap-1 sm:flex-row">
      ...
      {/if}
      <div
        class="flex w-full flex-row flex-wrap items-end justify-end gap-1 pr-3"
      >
        ...
        {/if}
        <div
          class="flex h-10 items-center justify-center gap-1 rounded-md border-2 px-2"
        >
              ...
              >
                <Checkbox
                  ...
                  checked={selectedValues.has(
                    getObjectValues(row, dataKey ?? "code")
                  )}
                />
              </td>
            {/if}
      ...
      <tfoot>
        ...
        >
          {#if isSelectable}
            <td
              class={cn(
                "flex-shrink-0 p-2 text-center align-middle [&:has([role=checkbox])]:pr-0"
              )}
            >
            </td>
          {/if}

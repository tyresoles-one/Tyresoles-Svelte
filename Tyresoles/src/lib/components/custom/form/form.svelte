<script lang="ts">
  ...
  onMount(() => {
    ...
    if (autofocus) {
      ...
      if (focusableElements.length > 0) {
        console.log(focusableElements[0], "focuseable [0]");
        setTimeout(() => {
  ...
  function handleSubmit(event: Event) {
    ...
    if (
      Object.values(errors).every((v) => Array.isArray(v) && v.length === 0)
    ) {
      const numberFields = props.fields.filter(
...
>
  ...
  >
    ...
    <div
      class="flex w-full flex-grow justify-between gap-2 overflow-x-auto px-4 pb-4"
    >
      {#if props.actions}

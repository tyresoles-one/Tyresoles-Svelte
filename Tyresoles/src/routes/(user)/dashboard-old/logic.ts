...
export const prepareBusinessLocations = (
  input: DashboardData,
  activeView: string
): BusinessLocation[] => {
...
export const prepareData = (
  ...
  if (input && input.name) {
    switch (input.name) {
      ...
      case "SalesmanSale":
      case "DealerSale":
        return prepareDataSalesman(input, busLocations);
      default:
        return input;
    }
  }
...
const prepareDataProductSale = (
    ...
    for (const defLocData of defaultLocData) {
      ...
      defLocData.data.forEach((element) => {
        ...
        filtered.forEach((flElement) => {
          filteredSales.push(
            ...flElement.data.filter((fle) => fle.label === element.label)
          );
        });
  ...
  busLocations.forEach((busLoc) => {
    ...
    firstData.data.forEach((f) => {
      ...
      locData
        .filter((fl) => !fl.product.includes("Intercompany"))
        .forEach((ld) => {
      ...
      const newSale: Sales = {
        ...
        saleText:
          "" +
          (sumBy(filtered, "sale") === 0
            ? "-"
            : (sumBy(filtered, "sale") / 100000).toFixed(2)),
        items: [],
      };
  ...
  busLocations.forEach((busLoc) => {
    ...
    for (const defLocData of defaultLocData) {
      ...
      defLocData.data.forEach((element) => {
        ...
        filtered.forEach((flElement) => {
          filteredSales.push(
            ...flElement.data.filter(
              (fle) => fle.dateRange === element.dateRange
            )
          );
        });
...
const prepareDataSalesman = (
  ...
  busLocations.forEach((busLoc) => {
    ...
    for (const defLocData of defaultLocData) {
      let newSalesmanSale: SalesmanSale = { ...defLocData, data: [] };
      ...
      filtered.forEach((fd) => {
        if (fd && fd.data && fd.data.length > 0) newData.push(...fd?.data);
      });
